/**
 * Renderiza community.html (5 scenes, multi-audio per scene) a MP4.
 * Adaptado de render-challengersranking.js (mismo patrón):
 *   - 5 escenas
 *   - localStorage key: cr-community-lang   (community usa otra key)
 *   - audio dir: community-audio/{lang}/
 *
 * Uso: node scripts/render-community.js <es|en> <output.mp4>
 *   p.ej: node scripts/render-community.js es community-es.mp4
 */
...
if (!lang || !outputMp4 || (lang !== 'es' && lang !== 'en')) {
  console.error('Uso: node scripts/render-community.js <es|en> <output.mp4>');
  process.exit(1);
}

const W = 1080, H = 1920;
const LANDING = '/Users/bernardoloitegui/Challengers Landing';
const SALES_HTML = `${LANDING}/community.html`;
const AUDIO_DIR = `${LANDING}/community-audio/${lang}`;
...
async function main() {
  const workDir = await fs.mkdtemp(path.join(os.tmpdir(), 'community-render-'));
  ...
  // Preset lang en localStorage (community usa cr-community-lang)
  await page.addInitScript((l) => {
    try { localStorage.setItem('cr-community-lang', l); } catch {}
  }, lang);
