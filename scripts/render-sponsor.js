/**
 * Renderiza sponsor.html (8 scenes, multi-audio per scene) a MP4.
 * Adaptado de render-community.js (mismo patrón):
 *   - 3 escenas
 *   - localStorage key: cr-sponsor-lang
 *   - audio dir: sponsor-audio/{lang}/
 *
 * Uso: node scripts/render-sponsor.js <es|en> <output.mp4>
 */
...
if (!lang || !outputMp4 || (lang !== 'es' && lang !== 'en')) {
  console.error('Uso: node scripts/render-sponsor.js <es|en> <output.mp4>');
  process.exit(1);
}

const W = 1080, H = 1920;
const LANDING = '/Users/bernardoloitegui/Challengers Landing';
const SALES_HTML = `${LANDING}/sponsor.html`;
const AUDIO_DIR = `${LANDING}/sponsor-audio/${lang}`;
const SCENES = ['01','02','03'];
const LANG_KEY = 'cr-sponsor-lang';
...
async function main() {
  const workDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sponsor-render-'));
  ...
  await page.addInitScript((l) => { try { localStorage.setItem('cr-sponsor-lang', l); } catch {} }, lang);
