...
const SALES_HTML = `${LANDING}/newman-welcome.html`;
const AUDIO_DIR = `${LANDING}/newman-welcome-audio/${lang}`;
...
async function main() {
  const workDir = await fs.mkdtemp(path.join(os.tmpdir(), 'newman-welcome-render-'));
  ...
  // Marca el inicio de la grabación. El lead-in (flash blanco de carga +
  // play-overlay) ocurre entre acá y el click de play. Lo medimos para
  // recortarlo SOLO del video más abajo, así el MP4 abre directo en la
  // escena 1 (sin frame blanco que WhatsApp tome de thumbnail).
  const recStart = Date.now();

  // Preset lang en localStorage (community usa cr-cr-video-lang)
  await page.addInitScript((l) => {
    try { localStorage.setItem('cr-newman-welcome-lang', l); } catch {}
  }, lang);
  ...
  // Lead-in a recortar = tiempo desde el inicio de grabación hasta justo
  // antes de arrancar las scenes. La escena 1 empieza a animar en el click,
  // por lo que su fade-in coincide con el inicio del audio "¡Bienvenido…".
  ...
  await ffrun([
    '-y',
    '-ss', leadSec.toFixed(3), '-i', webm,   // recorta el lead-in SOLO del video
    '-i', combinedAudio,                      // audio desde 0 → sincroniza scene 1
